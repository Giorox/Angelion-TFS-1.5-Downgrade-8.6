...
local function checkArea(pos, monster)
    -- Get all "spectators", any entity (NPC, Monster, Player) in a certain radius of POS
	local spectators = getSpectators(pos, 2, 2) -- 2 tile left, 2 right, 2 up, 2 down
	
    -- If anything is present, check if any of them are monsters, and if they are monsters, if they are `monster` by checking the name
...
function onThink(interval)
    -- Choose a boss to spawn at random from the table
